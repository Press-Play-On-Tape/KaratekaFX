...
void renderPlayerStance(Arduboy2Ext &arduboy, int8_t x, int8_t y, uint8_t stance) {
  ...
  uint24_t image = FX::readIndexedUInt24(Images::Bodies, body.index);
  if (body.index != 127) FX::drawBitmap(x + body.xOffset, y + body.yOffset, image, body.rev, dbmMasked);
  // if (head.index != 127) FX::drawBitmap(x + head.xOffset, y + head.yOffset, Images::Head1, 0, dbmMasked);
  if (head.index != 127) FX::drawBitmap(x + head.xOffset - 1, y + head.yOffset - 1, Images::Head1, 0, dbmMasked);
